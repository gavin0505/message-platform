...
public class PreParamCheckAction implements BusinessProcess<SendTaskModel> {
    ...
    public void process(ProcessContext<SendTaskModel> context) {
        ...
        // 1.若没有传入 消息模板Id 或者 messageParam
        if (messageTemplateId == null || CollUtil.isEmpty(messageParamList)) {
            context.setNeedBreak(true).setResponse(ResultVO.error(RespStatusEnum.CLIENT_BAD_PARAMETERS.getCode(), RespStatusEnum.CLIENT_BAD_PARAMETERS.getMsg()));
            return;
        }
        ...
        if (messageParamList.stream().anyMatch(messageParam -> messageParam.getReceiver().split(StrUtil.COMMA).length > BATCH_RECEIVER_SIZE)) {
            context.setNeedBreak(true).setResponse(ResultVO.error(RespStatusEnum.TOO_MANY_RECEIVER.getCode(), RespStatusEnum.TOO_MANY_RECEIVER.getMsg()));
        }
    }
}
